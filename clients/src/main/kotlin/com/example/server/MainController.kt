...
class MainController(rpc: NodeRPCConnection) {
    ...
    fun createIOU(request: HttpServletRequest): ResponseEntity<String> {
        ...
        return try {
            val iouId = proxy.startFlowDynamic(CreateFlow.Initiator::class.java, iouValue, otherParty).returnValue.getOrThrow()
    ...
    @PostMapping(value = [ "payback" ], produces = [ TEXT_PLAIN_VALUE ], headers = [ "Content-Type=application/x-www-form-urlencoded" ])
    fun paybackIOU(request: HttpServletRequest): ResponseEntity<String> {

        val paybackValue = request.getParameter("paybackValue").toInt()
        val linearId = request.getParameter("linearId")
        if(paybackValue == null){
            return ResponseEntity.badRequest().body("Query parameter 'paybackValue' must not be null.\n")
        }
        if (linearId == null){
        ...
        return try {
            val iouId = proxy.startFlowDynamic(SettleFlow.Initiator::class.java, linearId, paybackValue).returnValue.getOrThrow()
            ResponseEntity.status(HttpStatus.CREATED).body("Payback ${iouId}.\n")
