...
app.controller('DemoAppController', function($http, $location, $uibModal) {
    ...
    demoApp.openModalPayback = () => {
            const modalInstance = $uibModal.open({
                templateUrl: 'demoAppModalPayback.html',
                controller: 'ModalInstancePaybackCtrl',
                controllerAs: 'modalInstance',
                resolve: {
                    demoApp: () => demoApp,
                    apiBaseURL: () => apiBaseURL,
...
app.controller('ModalInstancePaybackCtrl', function ($http, $location, $uibModalInstance, $uibModal, demoApp, apiBaseURL, peers) {
    const modalInstance = this;
    ...
    // Validates and payback IOU.
        modalInstance.payback = function validateAndSendIOU() {
            if (modalInstance.form.value <= 0) {
                modalInstance.formError = true;
            } else {
                modalInstance.formError = false;
                ...
                let PAYBACK_IOUS_PATH = apiBaseURL + "payback"

                let paybackIOUData = $.param({
                    linearId: modalInstance.form.linearId,
                    paybackValue : modalInstance.form.paybackValue

                });

                let paybackIOUHeaders = {
                    headers : {
                        "Content-Type": "application/x-www-form-urlencoded"
                    }
                };
                ...
                // Create IOU  and handles success / fail responses.
                $http.post(PAYBACK_IOUS_PATH, paybackIOUData, paybackIOUHeaders).then(
                    modalInstance.displayMessage,
                    modalInstance.displayMessage
                );
            }
